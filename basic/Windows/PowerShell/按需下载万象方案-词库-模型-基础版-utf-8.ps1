...
if ($SelectedDictRelease -and $SelectedSchemaRelease -and $SelectedGramRelease) {
    ...
    Write-Host "解析出最新的方案版本链接为：$($SelectedSchemaRelease.zipball_url)" -ForegroundColor Green
...
$ExpectedSchemaTypeInfo = $SelectedSchemaRelease
$ExpectedDictTypeInfo = $SelectedDictRelease.assets[0]
...
if ($InputSchemaDown -eq "0") {
    ...
    if ($Debug) {
        Write-Host "最新的方案下载信息为：$($ExpectedSchemaTypeInfo)" -ForegroundColor Green
    }
}
...
if ($InputDictDown -eq "0") {
    ...
    if ($Debug) {
        Write-Host "最新的词库下载信息为：$($ExpectedDictTypeInfo)" -ForegroundColor Green
    }
}
...
if ($InputGramModel -eq "0") {
    ...
    if ($Debug) {
        Write-Host "最新的模型下载信息为：$($ExpectedGramTypeInfo)" -ForegroundColor Green
    }
}
...
# 下载函数
function Download-BasicSchemaFiles {
    param(
        [Object]$assetInfo,
        [string]$outFilePath
    )
    ...
    try {
        $downloadUrl = $assetInfo.zipball_url
        Write-Host "正在下载文件:$($assetInfo.name)..." -ForegroundColor Green
        Invoke-WebRequest -Uri $downloadUrl -OutFile $outFilePath -UseBasicParsing
        Write-Host "下载完成" -ForegroundColor Green
    }
...
if ($InputSchemaDown -eq "0") {
    ...
    if (Compare-UpdateTime -localTime $SchemaUpdateTime -remoteTime $SchemaRemoteTime) {
        ...
        Download-BasicSchemaFiles -assetInfo $ExpectedSchemaTypeInfo -outFilePath $tempSchemaZip
        ...
        $sourceDir = Get-ExtractedFolderPath -extractPath $SchemaExtractPath -assetName $KeyTable["0"]
...
if ($InputDictDown -eq "0") {
    ...
    if (Compare-UpdateTime -localTime $DictUpdateTime -remoteTime $DictRemoteTime) {
        ...
        Write-Host "正在复制文件..." -ForegroundColor Green
        $sourceDir = Get-ExtractedFolderPath -extractPath $DictExtractPath -assetName $KeyTable["1"]
        if (-not (Test-Path $sourceDir)) {
            Write-Host "错误：压缩包中未找到 $sourceDir 目录" -ForegroundColor Red
        ...
        Start-Sleep -Seconds 1
        # 删除目标目录
        if (Test-Path -Path $(Join-Path $targetDir "cn_dicts")) {
            Remove-Item -Path $(Join-Path $targetDir "cn_dicts") -Recurse -Force
        }
